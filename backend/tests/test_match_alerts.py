...
class TestCustomAlertThreshold:
    """User-configurable alert threshold overrides the default 10%."""

    @patch("app.services.email_orchestrator.settings")
    @patch("app.services.email_orchestrator.send_email", return_value="msg_123")
    def test_custom_threshold_5pct_triggers_at_5(self, mock_send, mock_settings, db):
        """User with 5% threshold sees 'Price drop:' on a 5% drop."""
        mock_settings.EMAIL_V2_ENABLED = True
        mock_settings.EMAIL_DRY_RUN = True
        mock_settings.EMAIL_SUSPEND_NONCRITICAL = False

        user = _make_user(db)
        user.alert_threshold = 5
        db.flush()

        # Previous min $1000, now $950 = 5% drop
        signal = _make_signal(db, user, last_check_min_price=100000, all_time_low_price=95000)
        run_id = str(uuid.uuid4())
        deals = [_deal_dict(price_cents=95000)]
        process_signal_matches(db=db, signal_deals={str(signal.id): deals}, run_id=run_id)
    ...
    @patch("app.services.email_orchestrator.settings")
    @patch("app.services.email_orchestrator.send_email", return_value="msg_123")
    def test_custom_threshold_20pct_ignores_10pct_drop(self, mock_send, mock_settings, db):
        """User with 20% threshold does NOT get 'Price drop:' on a 10% drop."""
        mock_settings.EMAIL_V2_ENABLED = True
        mock_settings.EMAIL_DRY_RUN = True
        mock_settings.EMAIL_SUSPEND_NONCRITICAL = False

        user = _make_user(db)
        user.alert_threshold = 20
        db.flush()

        # Previous min $1000, now $900 = 10% drop (below user's 20% threshold)
    ...
    @patch("app.services.email_orchestrator.settings")
    @patch("app.services.email_orchestrator.send_email", return_value="msg_123")
    def test_default_threshold_is_10(self, mock_send, mock_settings, db):
        """Users with default alert_threshold=10 behave identically to old logic."""
        mock_settings.EMAIL_V2_ENABLED = True
        mock_settings.EMAIL_DRY_RUN = True
        mock_settings.EMAIL_SUSPEND_NONCRITICAL = False

        user = _make_user(db)
        assert user.alert_threshold == 10
...
class TestPctDropThresholds:
    """Percentage drop preview: >= 10% triggers, < 10% does not (default threshold)."""
...
class TestQuietHoursSuppression:
    """Quiet hours suppresses ALERT emails when in the user's quiet window."""

    @patch("app.services.email_orchestrator.settings")
    @patch("app.services.email_orchestrator.send_email", return_value="msg_123")
    def test_quiet_hours_suppresses_during_window(self, mock_send, mock_settings, db):
        """Alert is suppressed when current time is within quiet hours."""
        mock_settings.EMAIL_V2_ENABLED = True
        mock_settings.EMAIL_DRY_RUN = False
        mock_settings.EMAIL_SUSPEND_NONCRITICAL = False
        ...
        # Patch _in_quiet_hours to return True (simulating nighttime)
        with patch("app.services.email_orchestrator._in_quiet_hours", return_value=True):
            results = process_signal_matches(
                db=db, signal_deals={str(signal.id): deals}, run_id=run_id,
            )

        assert results[0]["status"] == "suppressed"
        assert results[0]["reason"] == "quiet_hours"

    @patch("app.services.email_orchestrator.settings")
    @patch("app.services.email_orchestrator.send_email", return_value="msg_123")
    def test_quiet_hours_allows_outside_window(self, mock_send, mock_settings, db):
        """Alert is sent when current time is outside quiet hours."""
        mock_settings.EMAIL_V2_ENABLED = True
        mock_settings.EMAIL_DRY_RUN = False
        mock_settings.EMAIL_SUSPEND_NONCRITICAL = False
        ...
        # Patch _in_quiet_hours to return False (simulating daytime)
        with patch("app.services.email_orchestrator._in_quiet_hours", return_value=False):
            results = process_signal_matches(
                db=db, signal_deals={str(signal.id): deals}, run_id=run_id,
            )
    ...
    @patch("app.services.email_orchestrator.settings")
    @patch("app.services.email_orchestrator.send_email", return_value="msg_123")
    def test_quiet_hours_disabled_sends_always(self, mock_send, mock_settings, db):
        """With quiet_hours_enabled=False, alerts always send."""
        mock_settings.EMAIL_V2_ENABLED = True
        mock_settings.EMAIL_DRY_RUN = False
        mock_settings.EMAIL_SUSPEND_NONCRITICAL = False
...
class TestInQuietHoursUnit:
    """Unit tests for _in_quiet_hours logic."""

    def test_within_overnight_window(self):
        """23:00 is within 21:00-08:00 window."""
        from app.services.email_orchestrator import _in_quiet_hours

        user = User(
            id=uuid.uuid4(), clerk_id="test", email="t@t.com",
            quiet_hours_enabled=True, timezone="UTC",
            quiet_hours_start="21:00", quiet_hours_end="08:00",
        )
        # Mock datetime to return 23:00 UTC
        with patch("app.services.email_orchestrator.datetime") as mock_dt:
            from datetime import datetime as real_dt
            from zoneinfo import ZoneInfo
            mock_dt.now.return_value = real_dt(2026, 3, 4, 23, 0, tzinfo=ZoneInfo("UTC"))
            assert _in_quiet_hours(user) is True

    def test_outside_overnight_window(self):
        """14:00 is outside 21:00-08:00 window."""
        from app.services.email_orchestrator import _in_quiet_hours
        ...
        with patch("app.services.email_orchestrator.datetime") as mock_dt:
            from datetime import datetime as real_dt
            from zoneinfo import ZoneInfo
            mock_dt.now.return_value = real_dt(2026, 3, 4, 14, 0, tzinfo=ZoneInfo("UTC"))
            assert _in_quiet_hours(user) is False
    ...
    def test_within_daytime_window(self):
        ...
        with patch("app.services.email_orchestrator.datetime") as mock_dt:
            from datetime import datetime as real_dt
            from zoneinfo import ZoneInfo
            mock_dt.now.return_value = real_dt(2026, 3, 4, 10, 0, tzinfo=ZoneInfo("UTC"))
            assert _in_quiet_hours(user) is True

    def test_no_timezone_returns_false(self):
        """Missing timezone should not suppress (returns False)."""
        from app.services.email_orchestrator import _in_quiet_hours

        user = User(
            id=uuid.uuid4(), clerk_id="test", email="t@t.com",
            quiet_hours_enabled=True, timezone=None,
            quiet_hours_start="21:00", quiet_hours_end="08:00",
        )
        # timezone is None so _in_quiet_hours won't be called in suppression check,
        # but if called directly it should handle gracefully
        assert _in_quiet_hours(user) is False
